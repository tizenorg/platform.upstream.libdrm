...
#endif
#define LOADER_ERROR(fmt, ...) fprintf(stderr, fmt, ##__VA_ARGS__)
...
int loader_load_module (const char *name, const char *busid)
{
	...
	for (i = 0; i < ARRAY_SIZE(support_modules_list); i++) {
		if ((lib_handler = open_module (support_modules_list[i], RTLD_LAZY|RTLD_GLOBAL)) == NULL) {
			LOADER_ERROR("[DRM_LOADER] %s\n", dlerror());
			continue;
		}
		...
		if ((fd = init_device_func(name)) > 0) {
			LOADER_ERROR("[DRM_LOADER] Load libs %s\n", name);
			return fd;
		}
		LOADER_ERROR("fd is %d\n",fd);
		dlclose(lib_handler);
		lib_handler = NULL;
	}
	LOADER_ERROR("[DRM_LOADER] Can't load libs\n");
	return -1;
}
